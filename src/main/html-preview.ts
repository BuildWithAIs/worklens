...
export async function readHtmlPreview(
  root: string | string[],
  file: string,
  ...
  const bases = await Promise.all(
    (Array.isArray(root) ? root : [root]).map((path) =>
  ...
  const within = (base: string) => {
    const part = relative(base, actual);
    return (
      !!part &&
      part !== ".." &&
      !part.startsWith(".." + sep) &&
  ...
  if (!bases.some((base) => base && within(base)) || actual !== resolve(file))
    throw unavailable();
...
export async function htmlActionFile(
  root: string | string[],
  ...
  if (
    ...
    !messages.some(
      (message) => message.role === "assistant" && message.text.includes(code),
    )
  )
    throw new Error("HTML source unavailable");
  const directory = await mkdtemp(join(outputDirectory, "worklens-html-"));
