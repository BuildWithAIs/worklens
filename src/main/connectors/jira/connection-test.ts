...
import { JiraHttp, ServiceError } from "./http";
...
export async function testConnection(
  ...
  const http = new JiraHttp(snapshot, signal, fetcher);
  if (
  ...
  const user = await http.json(
    `/rest/api/${snapshot.settings.deployment === "cloud" ? 3 : 2}/myself`,
  );
  if (
    user.type === "anonymous" ||
    ![user.accountId, user.name, user.key].some(
  ...
  return `已连接：${user.displayName ?? user.name ?? user.accountId} · ${snapshot.settings.url}`;
}
