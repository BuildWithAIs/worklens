...
export interface ConnectionSnapshot {
  settings: JiraConnection;
  token: string;
  revision: string;
  signal: AbortSignal;
}
export function normalizeSettings(raw: JiraSettingsInput): JiraSettingsInput {
  ...
  if (
    ...
    throw new Error("请输入不含凭据、查询参数或片段的 Jira 站点地址");
...
export class JiraConnections {
    ...
    try {
      ...
      try {
        ...
        this.value.error = this.redact(
          error instanceof Error ? error.message : "Jira 连接验证失败",
        );
      }
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT")
        this.value.error =
          "无法读取或解密 Jira 配置，原文件已保留。请重新填写 token 或断开连接。";
    }
  }
  info(): JiraConnection {
    return { ...this.value };
  ...
  async candidate(raw: JiraSettingsInput): Promise<ConnectionSnapshot> {
    const input = normalizeSettings(raw);
    if (
      input.deployment === "cloud" &&
      input.tokenType === "scoped" &&
      !input.cloudId
    ) {
      const response = await this.fetcher(
        new URL("/_edge/tenant_info", input.url),
        {
          redirect: "manual",
      ...
      if (!response.ok) {
        ...
        throw new Error("无法自动获取 Cloud ID，请手工填写");
      }
      ...
      if (
        ...
        !z
          ...
          .safeParse(tenant.cloudId).success
      )
        throw new Error("无法自动获取 Cloud ID，请手工填写");
      input.cloudId = tenant.cloudId;
    }
    const sameAccount =
  ...
  snapshot(): ConnectionSnapshot {
    if (!this.token || !this.value.configured)
      throw new Error("请先在设置 → 连接中保存并验证 Jira 连接");
    return {
      settings: this.info(),
      token: this.token,
      revision: this.revision,
  ...
  assertCurrent(snapshot: ConnectionSnapshot) {
    ...
    if (snapshot.revision !== this.revision)
      throw new Error("Jira 连接已变更，请重新读取目标");
  }
  async test(input: JiraSettingsInput) {
    return testConnection(await this.candidate(input), this.fetcher);
  }
  private disable() {
    this.controller.abort(new Error("Jira 连接待验证或已失效"));
    this.controller = new AbortController();
    this.value.configured = false;
  }
  save(input: JiraSettingsInput) {
    return this.queue.run("connection", async () => {
      ...
      this.controller.abort(new Error("Jira 连接已变更"));
      this.controller = new AbortController();
      this.revision = revision;
  ...
  remove() {
    return this.queue.run("connection", async () => {
      ...
      this.controller.abort(new Error("Jira 连接已断开"));
      this.controller = new AbortController();
