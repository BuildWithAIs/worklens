...
- `confluence/index.ts` and `jira/index.ts` adapt each service to this interface. Their domain modules stay together; they are not promoted to generic infrastructure just because another connector may eventually need similar functionality.
...
See [Confluence behavior and validation](confluence.md) and [Jira behavior, capability matrix and validation](jira.md).
